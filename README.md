...
1. `git clone git@github.com:josephdpurcell/drupal8-vagrant.git dev.local`
1. `cd dev.local`
1. `vagrant up`
1. `vagrant ssh`
1. `cd /var/www/dev.local`

You can now visit [http://dev.local](http://dev.local).
...
To create multiple vms (e.g. a dev.local and a dev2.d8.local):
